import 'package:CWCFlutter/features/Home/Presentation/bloc/home_bloc.dart';
import 'package:CWCFlutter/features/Home/Presentation/widgets/Home.dart';
import 'package:CWCFlutter/features/gmap/Presentation/Widgets/gmap.dart';
import 'package:CWCFlutter/features/gmap/Presentation/bloc/poly_bloc.dart';
import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';

import '../../../../Injection_Container.dart';
import '../../../signin/domain/entity/user.dart';

class PolyPage extends StatefulWidget {
  final User user;

  const PolyPage({Key key, this.user}) : super(key: key);
  @override
  _PolyPageState createState() => _PolyPageState();
}

class _PolyPageState extends State<PolyPage> {
  User user;
  @override
  Widget build(BuildContext context) {
    user = widget.user;
    return Scaffold(
        backgroundColor: Colors.white,
        body: BlocProvider(
            builder: (_) => sl<PolyBloc>(), child: GMap(user: user)));
  }
}
